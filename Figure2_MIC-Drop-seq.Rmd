...
DEseq_pbulk <- function(so, resolution = "curated_cell_type2"){
  ...
  for(mutation in unique(so$class)){  # the for loop iterates over each perturbation and calculates DEGs in each cluster for each target
    ...
    for(celltype in unique(celltypes)){
      ...
      cluster_metadata <- so@meta.data %>% 
        filter(curated_cell_type2 == celltype) %>%
        select(sample, group, curated_cell_type2, test_gene) %>% 
        group_by(group, curated_cell_type2) %>%
        dplyr::count(test_gene) %>%
        mutate(sample = paste0(curated_cell_type2,"_", test_gene, "_", group)) %>%
        arrange(factor(sample, levels = colnames(subexp))) %>% #ensure 
        column_to_rownames(var = "sample") 
      
      dds <- DESeqDataSetFromMatrix(subexp, 
...
```{r}
  ...
  meox <- subset(micdrop, subset = class != "ND") #Frist subset to remove ND cells 

    
    meox@meta.data <- mutate(meox@meta.data, test_gene = case_when(
      class == "meox1" ~ "meox1",
      TRUE ~ "Other"
    ))
    ...
    exp <- AggregateExpression(meox, assays = "RNA", group.by = c("curated_cell_type2","test_gene","group")) ## use seurat AggregateExpression to make pseudobulk count matrixes 
    
    exp <- exp$RNA
    
      
      subexp <- exp[,colnames(exp)[grepl(paste0("^","Myotome","_"), colnames(exp))]]
      subexp
      
    cluster_metadata <- meox@meta.data %>% 
      filter(curated_cell_type2 == "Myotome") %>%
      select(sample, group, curated_cell_type2, test_gene) %>% 
      group_by(group, curated_cell_type2) %>%
      dplyr::count(test_gene) %>%
      mutate(sample = paste0(curated_cell_type2,"_", test_gene, "_", group)) %>%
      arrange(factor(sample, levels = colnames(subexp))) %>%
      column_to_rownames(var = "sample") 
...
as.data.frame(res) %>% arrange(padj) %>% filter(padj < 0.05) %>% rownames_to_column("gene") 

# Shrink the log2 fold changes with apeglm
res <- lfcShrink(dds, 
                 coef = "test_geneOther",
                 res = res,
                 type = "apeglm")
...
```
test wilcox ranked sum results
```{r}

meox_myo <- subset(meox, subset = curated_cell_type2 == "Myotome")

Idents(meox_myo) <- "test_gene"

DimPlot(meox_myo, group.by = "test_gene")

default_seurat <- FindMarkers(meox_myo, ident.1 = "Other", ident.2 = "meox1", test.use = "DESeq2")
...
deg_helper_fn <- function(results,so,resolution){
  ...
  resolution_sym <- sym(resolution)
    ...
    ncells <- so@meta.data %>% 
      filter(class != "ND") %>%
      count(class,!!resolution_sym) %>%
      rename("perturbation" = "class","cell_type" = resolution, "ncell_perturbed" = "n")
...
##saveRDS(file = "/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/DEG_results.rds")
```

```{r}

## make dataframe of the significant nDEG for each cell type (over 800 cells total)

totaldeg_comp <- DEG_results %>% 
  filter(FDR < 0.05, ncell_perturbed > 10, abs(logFC) > 0.5, cluster_ncell > 800) %>%
...
ha = HeatmapAnnotation(foo = anno_block(gp = gpar(fill = c("#B2B8E0","#FFCD82","#85C989")), 
  labels = c("Neural","Mesoderm / Endoderm","Non-neural Ectoderm")))
                       
split <- factor(
  c(rep("Neural", 23), rep("Mesoderm / Endoderm", 22), rep("Non-neural Ectoderm", 10)),
...
png(filename = "/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/Figure 2/Figure_2C.png", height = 10, width = 13, units = "in", res = 300 )

hm <- Heatmap( totaldeg_comp, 
                 row_names_gp = gpar(fontface = "italic"),
                 na_col = "darkgrey",  col = pal, column_names_rot = 45, border = "lightgrey", 
                 column_split = split, top_annotation = ha, column_gap = unit(4,"mm"),
                 heatmap_legend_param = list(title = "nDEG"), rect_gp = gpar(col = "lightgrey"), cluster_column_slices = FALSE, 
                 row_dend_side = "right", row_names_side = "left", row_title =  "                  ", row_title_rot = 45)
hm
...
ggsave(device = "eps", width = 8, height = 3, units = "in", filename = "FigureS6C.eps")
...
write.csv(top_any, file = "top_any.csv")
...
ggsave(device = "eps", width = 8, height = 4, units = "in", filename = "FigureS7B.eps")
...
ggsave(device = "eps", width = 8, height = 4, units = "in", filename = "FigureS7A.eps")
...
# Function to create the common plot with shared scales
common_plot <- function(data, hide_y_axis_labels = FALSE, tissue = "label", tiss_color = "grey", FDR_th = 0.05, logFC_th = 0.5) {
  ## For the purposes of plotting, we filter non-significant datapoints (set to 0) by FDR and logFC thresholds
  data <- data %>% 
  mutate(signed_qval = case_when(
    abs(signed_qval) < -log10(FDR_th) ~ 0,
    TRUE ~ signed_qval
  ),
  delta_log_abund = case_when(
    abs(signed_qval) < -log10(FDR_th) ~ 0,
    abs(delta_log_abund) < logFC_th ~ 0,
    TRUE ~ delta_log_abund
  ))
  print(data)

  plot <- ggplot(data, aes(x = cell_type, y = perturbation)) + 
    geom_point(aes(fill = signed_qval, size = abs(delta_log_abund)), alpha = 0.75, shape = 21) + 
    scale_size_continuous(limits = c(0.5, 3), range = c(4,30), guide = FALSE, name = "log(FC)") +
    scale_fill_gradientn(colors = c("blue","white","red"), guide = FALSE) + 
    theme_minimal() +
    theme(
  ...
  if (hide_y_axis_labels) {
    plot <- plot + theme(axis.text.y = element_blank())  # Hide y-axis text labels
  }
...
plot_part1 <- common_plot(hooke_part1, tissue = "Neural", tiss_color = "#B2B8E0") 
plot_part2 <- common_plot(hooke_part2, tissue = "Mesoderm / Endoderm", tiss_color = "#FFCD82", hide_y_axis_labels = TRUE)
plot_part3 <- common_plot(hooke_part3 ,tissue = "Non-neural Ectoderm", tiss_color = "#85C989", hide_y_axis_labels = TRUE) +
...
final_plot <- plot_part1 + plot_part2 + plot_part3
...
ggsave("/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/Figure 2/Figure_2D_3.png", 
       device = "png",
       width = 3.5,
       height = 9.5)
```
...
png(filename = "/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/Figure 2/Figure_2D.png", height = 10, width = 13, units = "in", res = 300 )
...
```{r}
hooke <- readRDS( file = "/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/MIC-Drop-seq/hooke_4_19_24.rds")
```

```{r}
hooke_subset %>% filter(perturbation == "tp63")
hooke %>% filter(delta_q_value < 0.05)
```


```{r}
##make vector of col names from DEG heatmap
perturb_order <- hm@column_names_param$labels[column_order(hm)]
#Filter to only include cell types that were in DEG heatmap (>800 cells in cluster)
...
hooke_subset <- hooke_subset %>% 
  filter(cell_type %in% hooke_significant_ct) %>%
  mutate(perturbation = factor(perturbation, levels =(dimnames(hm@matrix)[[2]])))
...
size_mat <- size_mat[dimnames(hm@matrix)[[2]], ct_order]
spacer <- matrix(NA, nrow = nrow(size_mat), ncol = 1)  # Spacer column (could use 0 instead of NA)
colnames(spacer) <- " "  # Name the spacer column
size_mat <- cbind(size_mat[, 1:13], spacer, size_mat[, 14:29], spacer, size_mat[,30:37])  # Add spacer after the second column
size_mat <- t(size_mat)
size_mat <- size_mat[, perturb_order]
...
color_mat <- color_mat[dimnames(hm@matrix)[[2]], ct_order]
color_mat <- cbind(color_mat[, 1:13], spacer, color_mat[, 14:29], spacer, color_mat[,30:37])  # Add spacer after the second column
color_mat <- t(color_mat)  
color_mat <- color_mat[,perturb_order]


png(filename = "/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/Figure_code/Figure 2/Figure_2D.png", height = 10, width = 14, units = "in", res = 300 )

bubble <- bubbleHeatmap(colorMat = color_mat, sizeMat = size_mat, legendTitles = c(expression("Cell abundance\nfold change"), "Signed\nlog(q)"), diameter = 1.1, showTopLabels = TRUE)

bubble$children$TopLabels$just <- c("right","top")

bubble$children$TopLabels$y <- unit(-36, "lines")

bubble$children$LeftLabels$x <- unit(2, "lines")

bubble$children$GridSq$gp <- gpar(lwd = 1.2, col = "lightgrey")

bubble$children$TopLabels$gp <- gpar(fontface = "italic", fontfamily = "Arial", fontsize = 16)

bubble$children$LeftLabels$gp <- gpar(fontfamily = "Arial", fontsize = 16)
...
bubble$children$GridSq$width
```


```{r}
hooke_filtered <- rbind(hooke_part1, hooke_part2, hooke_part3)
hooke_filtered

hooke_filtered %>% filter(delta_q_value < 0.01 , abs(delta_log_abund) > 0.5) %>% select(perturbation, cell_type, delta_q_value, delta_log_abund, signed_qval)
```

```{r}
hooke2 <- total_DEG %>%
  select(cell_type, cluster_ncell, perturbation, ncell_perturbed) %>%
  right_join(hooke, by = c("cell_type","perturbation"))
hooke2

hooke2 %>% 
  distinct() %>%
  filter(perturbation == "tbxta")

clust_pert <- hooke2 %>% 
  distinct() %>%
  mutate(perturbation = factor(perturbation)) %>%
  filter(delta_q_value < 0.05, abs(delta_log_abund) > 0.5, cluster_ncell > 800) %>%
  dplyr::count(perturbation, .drop = FALSE) %>% 
  arrange(desc(n)) %>% 
  mutate(perturbation = factor(perturbation, levels = perturbation))
  

ggplot(clust_pert, aes(x = perturbation, y = n)) +
  geom_bar(stat = "identity", fill = "black") +  # Change bar color to black
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, max(clust_pert$n), 1), expand = c(0,0.1)) + # Set y-axis ticks to 1, 2, 3, 4, ...
  theme(panel.grid.major = element_blank()) +
  labs(x = "Mutation", y = "# Cell types altered", title = "Number of altered cell type abundances / mutant", caption = paste0("Median: ", median(clust_pert$n)))
```

```{r}

cells_highlight <- micdrop@meta.data %>%
  mutate(cells_highlight = ifelse(class == "rx3", "rx3", class)) %>%
  filter(cells_highlight == "rx3") %>%
  pull(cell_barcode)

micdrop@meta.data <- micdrop@meta.data %>%
  mutate(cells_highlight = ifelse(class == "tbxta", "tbxta", "other"))

micdrop@meta.data$cells_highlight

DimPlot(micdrop, group.by = cells_highlight)


dittoDimPlot(micdrop, var = "cells_highlight", order = "increasing", color.panel = c("grey", "red"))

```

```{r}

for(pert in unique(micdrop@meta.data$class)){
  
  if(pert != "ND"){
     micdrop@meta.data <- mutate(micdrop@meta.data, display = ifelse(class == pert, pert, "other"))
  }
  micdrop@meta.data <- micdrop@meta.data %>% mutate(display = factor(display, levels = c("other",pert)))
  Idents(micdrop) <- "display"
  DimPlot(micdrop, group.by = "display", cols = c("red","grey"), pt.size = 3, order = c("other",pert) ) + NoAxes() + ggtitle(paste0(pert))
  ggsave(filename = paste0("/Users/claytoncarey/Documents/Seurat/micdrop_seq/micdrop/autonomyscratch/perturbation_loop_gif/", pert,".png"), device = "png")
}
...
for(pert in unique(micdrop@meta.data$class)){
  ...
  micdrop@meta.data <- mutate(micdrop@meta.data, disp = ifelse(class == pert, pert, "other"))
  
  Meta_Highlight_Plot(micdrop, meta_data_column = "disp", meta_data_highlight = pert, highlight_color = "red", pt.size = 1, raster = FALSE) + NoLegend() + NoAxes() + ggtitle(pert)
...
FeaturePlot_scCustom(micdrop, features = "dlx1a", reduction = 'umap', raster = FALSE) + NoAxes()

```
```{r}
DA_stats <- hooke %>% 
  mutate(perturbation = factor(perturbation)) %>%
  filter(cluster_ncell > 800, abs(delta_log_abund) > 0.5, delta_q_value < 0.05) %>%
  dplyr::count(perturbation, .drop = FALSE) %>%
  arrange(desc(n)) %>%
  mutate(perturbation = factor(perturbation, levels = perturbation))
...
ggsave(device = "eps", width = 8, height = 3, units = "in", filename = "FigureS8.eps")
...
nDEG_DF <- DEG_results %>% 
  ...
  dplyr::count(cell_type, .drop = FALSE) %>% 
  rename("n" = "nDEG") %>%
  right_join(DEG_results, by = c("perturbation","cell_type")) %>%
  select(perturbation, cell_type, cluster_ncell, nDEG) %>%
  distinct() 

hooke %>% 
  left_join(nDEG_DF, by = c("cell_type", "perturbation")) %>% 
  filter(cluster_ncell.x > 800) %>% 
  ggplot(aes(x = nDEG, y = -log(delta_q_value))) + 
  geom_point()


```
```{r}
FeaturePlot_scCustom(micdrop, features = "neurod1", reduction = 'umap')
```

```{r}

VlnPlot(micdrop, features = 'neurod1', raster = FALSE) + NoLegend()
