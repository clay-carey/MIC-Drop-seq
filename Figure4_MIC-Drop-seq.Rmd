...
lin_pert_df <- map_dfr(unique(micdrop$class), function(pert) {
...
count_ncell <- meta %>% count(curated_cell_type2) %>% rename( "cell_type"  = "curated_cell_type2", "cluster_ncell" = "n") 

cell_class_df <- meta %>% select(cell_class, curated_cell_type2) %>% distinct() %>%  rename( "cell_type"  = "curated_cell_type2") 

DEG_results <- DEG_results %>% left_join(count_ncell, by = "cell_type") %>% left_join(cell_class_df, by = "cell_type")
